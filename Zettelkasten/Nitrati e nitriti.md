...
#### Muscolatura liscia di altri organi
-  È stato dimostrato sperimentalmente il rilassamento della muscolatura liscia bronchiale, di quella del tratto gastrointestinale (incluso il sistema biliare) e del tratto genitourinario
	- ==breve durata d'azione== → scarso significato clinico
...
- Essa contiene ione $Fe^{2+}$, che reagisce con lo *ione nitrito*, formando *metaemoglobina* (ione $Fe^{3+}$)
	- poiché essa ha bassa affinità per l'$O_2$, dosi elevate possono provocare **pseudo-cianosi**, **ipossia tessutale** e **morte**
- Fortunatamente, i livelli plasmatici di nitrito che si raggiungono con dosi elevate di nitrati organici o inorganici sono troppo bassi per causare una significativa emoglobinemia nell'adulto
>[!warning]
>Nel lattante, la flora intestinale è capace di convertire consistenti quantità di nitrato organico (che può essere presente, ad esempio, nell'acqua di pozzo) in ione nitrito. Si può, così, verificare un'accidentale esposizione a grandi quantità di ioni nitrito capaci di provocare gravi effetti tossici.
##### Nell'avvelenamento da cianuro
1. Quest'effetto ha ==valenza terapeutica== → legame dello ione $CN^-$ col ferro citocromiale
	- il $Fe^{2+}$ emoglobinico ha alta affinità  per il $CN^-$
	- la somministrazione di $NaNO_2$, subito dopo l'esposizione al cianuro, rigenererà il citocromo in forma attiva
2. La *cianoemoglobina* prodotta può essere ulteriormente detossificata da una somministrazione IV di tiosolfato di sodio ($Na_2S_2O_3$), che provoca la formazione di ione tiocianato ($SCN^-$)
	- ione meno tossico, che viene prontamente escreto
###### Idroxocobalamina
- Attualmente somministrata in preferenza di questa procedura antidotica
- Forma di vitamina B$_{12}$ avente anche un'alta affinità per il cianuro: lo trasforma in *cianocobalamina*
## Tossicità e abitudine
### Effetti sfavorevoli acuti
- Sono estensioni dirette della vasodilatazione terapeutica:
	- **ipotensione ortostatica**
	- **tachicardia**
	- **cefalea pulsante**
- In casi di aumentata pressione
	- intraoculare → possono essere usati anche in pazienti con [[glaucoma]]
	- intracranica → ==controindicati==
>[!warning]
>Di rado, cerotti di nitroglicerina transdermica hanno preso fuoco quando è stato applicato un elettroshock mediante un defibrillatore al torace di pazienti con defibrillazione ventricolare.
>- Questi cerotti devono essere **rimossi** prima dell'impiego di defibrillazioni esterni per **evitare ustioni superficiali**.
### Abitudine
- Continua[^4] esposizione ai nitrati → completa abitudine (tachifilassi) del muscolo liscio
- Meccanismi (probabilmente) implicati:
	- ridotta liberazione di NO → comparsa dell'abitudine alla nitroglicerina
	- compensazione sistemica:
		1. inizialmente, si verifica una ==scarica simpatica==
		2. dopo uno o più gg, la ==ritenzione di sali e acqua== può far vanificare le modificazioni emodinamiche terapeutiche indotte
- Non si verifica in modo uguale per tutti i donatori di nitrossido
	- es. il nitroprussiato rimane attivo per lunghi periodi ed altri nitrati organici appaiono meno suscettibili della nitroglicerina allo sviluppo di abitudine
...
### Somministrazione sublinguale
- A causa della rapida comparsa d'azione (1-3 min), è la ==più frequentemente impiegata== per il trattamento immediato dell'angina
- Dato che, però, la sua azione è breve (<= 20-30 min), non è un farmaco adatto per una terapia di mantenimento
>Gli effetti emodinamici dell'==isosorbide dinitrato== e altri fosfati inorganici - formulati in preparazioni sublinguali o masticabili - sono simili a quelli della nitroglicerina
### Somministrazione IV
- Anche in questo caso, la comparsa dell'azione è rapida (minuti), ma i suoi effetti emodinamici scompaiono altrettanto rapidamente con l'interruzione della somministrazione
- Impiego clinico ristretto al trattamento di gravi e ricorrenti crisi di angina a riposo
### Somministrazioni a lento assorbimento
>Efficacia limitata dallo sviluppo di abitudine (le linee guida suggeriscono di mantenere un intervallo di tempo senza nitrati di almeno 8h fra le dosi)
#### Attraverso la mucosa della bocca
- Da porre tra solco gengivale del mascellare superiore e guancia
#### Forma orale
#### Assorbimento transdermico
- Sebbene sia idonea a determinare livelli ematici di nitroglicerina anche per più di 24h, gli effetti emodinamici, di norma, non durano più di 6-8h
## Appendice: farmaci usati nel trattamento della disfunzione erettile
- È stata per lungo tempo soggetto di ricerca (da parte di amatori e studiosi di professione[^6]). Fra le sostanze usate in passato, generalmente discreditate, ci sono
	- "mosca spagnola" (irritante vescicale e uretrale)
	- ioimbina ($\alpha_2$-antagonista, vedi [[Farmaci antagonisti sui recettori adrenergici#Ioimbina|qui]])
	- noce moscata
	- miscele contenenti Pb, As e stricnina
### Erezione fisiologica
>Un processo analogo si verifica nei tessuti erettili femminili
- Richiede *rilassamento* della muscolatura liscia non vascolare dei corpi cavernosi, causando
	1. afflusso di sangue alla pressione arteriosa vicina nei seni della cavernosa
	2. la pressione del sangue causa l'erezione
- Avviene in risposta al rilascio di NO da [[Introduzione alla farmacologia del sistema nervoso autonomo#Neuroni non-adrenergici non-colinergici (NANC)|neuroni NANC]]
	- l'innervazione parasimpatica dev'essere intatta
	- la sintesi di NO dev'essere attiva
### Intervento farmacologico
- Alcuni altri rilassanti della muscolatura liscia (==analoghi della PGE1==, ==α-bloccanti==), se presenti in concentrazione abbastanza elevata, possono indipendentemente determinare rilassamento dei corpi cavernosi sufficiente per esitare in erezione
- Ogni farmaco aumentante il cGMP può risultare di valore nella disfunzione erettile se è presente un'innervazione normale
#### Sildenafil
- Aumenta il cGMP inibendone la demolizione ad opera della *PDE5*
- Ha avuto un grande successo commerciale perché assumibile per os
>[!info]
>Studi preclinici indicano che possa essere utile nel prevenire l'apoptosi e il rimodellamento cardiaco dopo ischemia e riperfusione.
##### Limite
- Di poco o scarso valore nell'uomo con perdita di potenza dovuta a lesione midollare o altri danni di innervazione ed in uomini privi di libido
- Ha anche un effetto sulla visione, causando *difficoltà nella discriminazione blu-verde*
##### Interazioni farmacologiche
- Potenzia l'azione di nitrati usati per trattare l'angina: sono stati segnalati **gravi ipotensioni** e qualche **[[infarto miocardico]]** in uomini assumenti i due farmaci
	- si raccomanda che passino almeno 6h fra l'uso di un nitrato e l'ingestione di sildenafil
##### Tadalafil e vardenafil
